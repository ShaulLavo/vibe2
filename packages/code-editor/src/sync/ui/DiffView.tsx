import { createSignal, Show, createMemo, For, createEffect } from 'solid-js'
...
export function DiffView(props: DiffViewProps) {
	const [mergedContent, setMergedContent] = createSignal(
		props.conflictInfo.localContent
	)

	createEffect(() => {
		setMergedContent(props.conflictInfo.localContent)
	})
	const [selectedConflictIndex, setSelectedConflictIndex] = createSignal<
		number | null
	>(null)
				...
				<div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-7xl w-full mx-4 max-h-[95vh] flex flex-col">
					<div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700 shrink-0">
						...
						<p class="text-sm text-gray-600 dark:text-gray-400 mt-1">
							Review the differences and manually merge the changes. Click on
							individual changes to accept them.
						</p>
					</div>
					...
					<div class="flex-1 overflow-hidden flex">
						<div class="flex-1 flex border-r border-gray-200 dark:border-gray-700">
							<div class="flex-1 border-r border-gray-200 dark:border-gray-700 flex flex-col">
								<div class="px-4 py-2 bg-green-50 dark:bg-green-900 border-b border-gray-200 dark:border-gray-700 shrink-0">
							...
							<div class="flex-1 flex flex-col">
								<div class="px-4 py-2 bg-blue-50 dark:bg-blue-900 border-b border-gray-200 dark:border-gray-700 shrink-0">
						...
						<div class="flex-1 flex flex-col">
							<div class="px-4 py-2 bg-purple-50 dark:bg-purple-900 border-b border-gray-200 dark:border-gray-700 shrink-0">
			...
			<For each={diffLines()}>
				{(diffLine) => (
					<div class={`flex ${getLineClass(diffLine.type)}`}>
						<div class="w-12 px-2 py-1 text-gray-500 dark:text-gray-400 text-right border-r border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 shrink-0">
							{diffLine.lineNumber}
						</div>
...
function MergeEditor(props: MergeEditorProps) {
	...
	const handleLineClick = (lineIndex: number) => {
		...
		const conflictIndex = conflicts.findIndex(
			(c) => lineIndex >= c.startLine && lineIndex <= c.endLine
		)
	...
	return (
		<div class="h-full flex">
			<div class="shrink-0 bg-gray-50 dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700">
				<For each={lines()}>
					{(_line, index) => {
						const lineNumber = () => index() + 1
						const isConflictLine = () => conflictLines().has(index())
						const conflictIndex = () => {
							const conflicts = props.conflicts.filter(
								(c) => c.type === 'conflict'
							)
							return conflicts.findIndex(
								(c) => index() >= c.startLine && index() <= c.endLine
							)
						}
						const isSelected = () =>
							conflictIndex() >= 0 &&
							conflictIndex() === props.selectedConflictIndex
