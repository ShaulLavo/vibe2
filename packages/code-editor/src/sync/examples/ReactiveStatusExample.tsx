 ...
 */
export function ReactiveStatusExample(props: { syncManager: EditorFileSyncManager }) {
			...
			<div class="p-4 space-y-6">
				...
				<section>
					...
					<div class="space-y-2">
						<For each={openFiles()}>
							{(filePath) => (
								<FileStatusRow filePath={filePath} />
							)}
						</For>
					</div>
				</section>
	...
	return (
		<div class="bg-blue-50 dark:bg-blue-900 p-4 rounded-lg">
			<div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
				<div>
					<div class="font-medium text-blue-800 dark:text-blue-200">Total Files</div>
					<div class="text-2xl font-bold">{aggregated().counts.total}</div>
				</div>
				<div>
					<div class="font-medium text-green-800 dark:text-green-200">Synced</div>
					<div class="text-2xl font-bold text-green-600">{aggregated().counts.synced}</div>
				</div>
				<div>
					<div class="font-medium text-orange-800 dark:text-orange-200">Modified</div>
					<div class="text-2xl font-bold text-orange-600">{aggregated().counts.dirty}</div>
				</div>
				<div>
					<div class="font-medium text-red-800 dark:text-red-200">Conflicts</div>
					<div class="text-2xl font-bold text-red-600">{aggregated().counts.conflicts}</div>
				</div>
			</div>
			...
			<div class="mt-3 pt-3 border-t border-blue-200 dark:border-blue-700">
				...
				<Show when={aggregated().hasIssues}>
					<div class="mt-2 text-sm text-red-700 dark:text-red-300">
						⚠️ {aggregated().counts.conflicts} conflicts and {aggregated().counts.errors} errors need attention
					</div>
				</Show>
			</div>
		</div>
 ...
 */
export function StatusChangeWatcherExample(props: { syncManager: EditorFileSyncManager }) {
	...
	return (
		<SyncStatusProvider syncManager={props.syncManager}>
			<div class="p-4">
				...
				{/* Example files that trigger notifications */}
				<div class="space-y-2">
					<StatusWatcherRow 
