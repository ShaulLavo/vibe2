...
const sortedTags = Array.from(discoveredTags).sort((a, b) =>
  a.localeCompare(b),
);
  ...
  return import(moduleUrl)
    .then((mod) => {
      ...
      if (mod.LOGGER_TOGGLE_DEFAULTS) {
        ...
        return new Map(Object.entries(defaults as Record<string, boolean>));
      }
      return new Map();
    })
    .catch(() => new Map());
...
function resolveModule(
  ...
  if (specifier.startsWith("./") || specifier.startsWith("../")) {
    ...
    const subpath =
      subpathParts.length > 0 ? subpathParts.join("/") : undefined;
