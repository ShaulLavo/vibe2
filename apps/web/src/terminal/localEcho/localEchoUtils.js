 ...
 */
export function getWordBoundaries(input, leftSide = true) {
    const boundaries = [];
    const wordRegex = /\w+/g;
    let match;
 ...
 */
export function collectAutocompleteCandidates(handlers, input) {
    const tokens = parse(input).filter((t) => typeof t === "string");
    ...
    const allCandidates = handlers.reduce((candidates, { fn, args }) => {
        try {
...
export function getSharedFragment(fragment, candidates) {
    if (candidates.length === 0)
        return null;
    if (fragment.length >= candidates[0].length)
        return fragment;
    const oldFragment = fragment;
    const nextChar = candidates[0].charAt(fragment.length);
    const newFragment = fragment + nextChar;
    for (const candidate of candidates) {
        if (!candidate.startsWith(oldFragment))
            return null;
        if (!candidate.startsWith(newFragment))
            return oldFragment;
    }
