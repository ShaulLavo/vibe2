 ...
 */
export function offsetToColRow(input: string, offset: number, maxCols: number): TerminalPosition {
	let row = 0
	let col = 0
...
export function collectAutocompleteCandidates(
	...
	const allCandidates = handlers.reduce<string[]>((candidates, { fn, args }) => {
		try {
