...
import type { EditorPane, TabState } from '../types'
			...
			fc.asyncProperty(
				...
				async (config) => {
					...
					try {
						...
						const newPaneId = layoutManager.splitPane(
							layoutManager.state.rootId, 
							'horizontal'
						)
						...
						layoutManager.updateTabState(layoutManager.state.rootId, tab1Id, {
							...
							selections: [], // Keep selections empty for simplicity
						})

						layoutManager.updateTabState(newPaneId, tab2Id, {
						...
						// Verify both tabs are using the same file (shared resources)
						expect(resourceManager.getTabCountForFile(config.filePath)).toBe(2)
						const buffer = resourceManager.getBuffer(config.filePath)
						expect(buffer).toBeDefined()

						// Get the actual tab states from the layout
						const pane1 = layoutManager.state.nodes[layoutManager.state.rootId] as EditorPane
						const pane2 = layoutManager.state.nodes[newPaneId] as EditorPane
						...
						expect(actualTab1!.state.cursorPosition.line).toBe(config.tab1State.cursorLine)
						expect(actualTab1!.state.cursorPosition.column).toBe(config.tab1State.cursorColumn)
						expect(actualTab2!.state.cursorPosition.line).toBe(config.tab2State.cursorLine)
						expect(actualTab2!.state.cursorPosition.column).toBe(config.tab2State.cursorColumn)
						...
						const statesAreDifferent = (
							...
							actualTab1!.state.cursorPosition.line !== actualTab2!.state.cursorPosition.line ||
							actualTab1!.state.cursorPosition.column !== actualTab2!.state.cursorPosition.column
						)
						expect(statesAreDifferent).toBe(true)

						// Verify both tabs show the same file content (shared buffer)
						expect(actualTab1!.content.filePath).toBe(config.filePath)
						expect(actualTab2!.content.filePath).toBe(config.filePath)
						expect(actualTab1!.content.filePath).toBe(actualTab2!.content.filePath)
