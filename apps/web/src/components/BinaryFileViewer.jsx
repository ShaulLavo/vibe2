...
const estimateRowHeight = (fontSize) => Math.max(Math.round(fontSize * LINE_HEIGHT_RATIO), MIN_ROW_HEIGHT);
const byteToHex = (byte) => byte.toString(16).padStart(2, "0").toUpperCase();
const BinaryRow = (props) => {
    const offset = () => props.virtualRow.index * BYTES_PER_ROW;
    const rowBytes = () => props.bytes.subarray(offset(), offset() + BYTES_PER_ROW);
    const asciiForByte = (byte) => byte >= 32 && byte <= 126 ? String.fromCharCode(byte) : ".";
    return (<div data-index={props.virtualRow.index} class="absolute left-0 right-0" style={{
      ...
      <div class="flex h-full items-center gap-4 px-3 text-xs text-zinc-100">
        ...
        <div class="flex-1">
          <div class="flex items-center gap-4">
            ...
            return (<>
                      <span class="cursor-default rounded px-1 tabular-nums text-zinc-200 font-normal" classList={{
                    "bg-emerald-400 text-zinc-900 font-semibold": isHovered(),
                }} onMouseEnter={() => props.setHoveredIndex(globalIndex())} onMouseLeave={() => props.setHoveredIndex(null)}>
                        {byteToHex(byte)}
                      </span>
            ...
            const globalIndex = () => offset() + idx();
            const isHovered = () => props.hoveredIndex() === globalIndex();
            return (<span class="cursor-default rounded px-1 text-zinc-200 font-normal" classList={{
                    "bg-emerald-400 text-zinc-900 font-semibold": isHovered(),
                }} onMouseEnter={() => props.setHoveredIndex(globalIndex())} onMouseLeave={() => props.setHoveredIndex(null)}>
                      {asciiForByte(byte)}
                    </span>);
...
export const BinaryFileViewer = (props) => {
    ...
    const totalRows = () => byteLength() > 0 ? Math.ceil(byteLength() / BYTES_PER_ROW) : 0;
    ...
    return (<div class="mt-4 flex-1 overflow-hidden rounded border border-zinc-800/70 bg-zinc-950/30 flex flex-col">
      <div class="border-b border-zinc-800/70 bg-zinc-900/60 px-3 py-2 text-[11px] uppercase tracking-[0.08em] text-zinc-400" style={{
            "font-family": props.fontFamily(),
        }}>
        <Show when={headerSummary()} fallback={<span>Binary file preview (hex + ASCII)</span>}>
          {headerSummary()}
        </Show>
      </div>

      <Show when={bytes() && byteLength() > 0} fallback={<div class="px-3 py-4 text-sm text-zinc-500">
        ...
        <div ref={(element) => {
          ...
          <div style={{
            ...
            <For each={virtualRows()}>
              {(virtualRow) => (<BinaryRow virtualRow={virtualRow} bytes={bytes()} rowHeight={rowHeight()} hoveredIndex={hoveredIndex} setHoveredIndex={setHoveredIndex}/>)}
            </For>
          </div>
        </div>
      </Show>
