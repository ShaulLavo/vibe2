...
import { registerLocalDirectoryFallback, unregisterLocalDirectoryFallback } from '../fallback/localDirectoryFallbackCoordinator';
// intentionally not exported from module
const FALLBACK_ERROR = 'local-directory-fallback-error';
export const LocalDirectoryFallbackDialog = () => {
    const [open, setOpen] = createSignal(false);
    const [reason, setReason] = createSignal('unsupported');
    const [error, setError] = createSignal();
    const [processing, setProcessing] = createSignal(false);
    const [pendingMode, setPendingMode] = createSignal();
    let pending = null;
    let fileInput;
    const reset = () => {
        setOpen(false);
    ...
    const handleRequest = (nextReason) => {
    ...
    const handleCancel = () => {
        if (pending) {
            pending.reject(new Error('Local directory fallback was cancelled by the user.'));
        }
        reset();
    };
    const handleFilesSelected = async (event) => {
        ...
        setError(undefined);
        try {
            const handle = mode === 'memory'
                ? await importDirectoryToMemory(files)
                : await importDirectoryToOpfs(files, {
                    confirmDestructive: () => window.confirm('This will replace your existing browser storage workspace. ' +
                        'All current files in the workspace will be permanently deleted. ' +
                        'Are you sure you want to continue?')
                });
            const nextSource = mode === 'memory' ? 'memory' : 'opfs';
    ...
    const buttonBase = 'inline-flex items-center justify-center rounded-md border px-3 py-2 text-sm font-medium transition focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400';
    ...
    return (<>
        ...
        }} class="hidden" multiple onChange={handleFilesSelected}/>
			<Dialog open={open()} onOpenChange={handleOpenChange} modal>
				<DialogContent class="bg-zinc-950/95 text-zinc-100">
					...
					<DialogFooter class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-end">
						<button type="button" class={`${buttonBase} border-zinc-600 bg-transparent text-zinc-200 hover:bg-zinc-800/80`} onClick={handleCancel} disabled={processing()}>
							Cancel
						</button>
						<button type="button" class={`${buttonBase} border-blue-500 bg-blue-600/20 text-blue-100 hover:bg-blue-500/30 disabled:opacity-60`} onClick={() => startImport('memory')} disabled={processing()}>
            ...
            : singleMode()
                ...
                : 'Load into Memory (temporary)'}
						...
						{!singleMode() && (<button type="button" class={`${buttonBase} border-emerald-500 bg-emerald-600/30 text-emerald-100 hover:bg-emerald-500/40 disabled:opacity-60`} onClick={() => startImport('opfs')} disabled={processing()}>
