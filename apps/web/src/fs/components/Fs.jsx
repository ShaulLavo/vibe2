...
export const Fs = () => {
    ...
    return (<div class="flex h-full min-h-0 flex-col overflow-hidden rounded-lg border border-zinc-800/70 bg-zinc-950/60 shadow-xl">
      ...
      <Resizable class="flex flex-1 min-h-0" orientation="horizontal" onSizesChange={(sizes) => {
            if (sizes.length !== 2)
                return;
            setPanelSizes(() => [...sizes]);
        }}>
        <ResizablePanel initialSize={panelSizes()[0] ?? 0.3} minSize={0.18} class="min-h-0 overflow-auto border-r border-zinc-800/70 bg-zinc-950/60" ref={treePanel}>
          <TreeView tree={() => state.tree} loading={() => state.loading}/>
        </ResizablePanel>
        <ResizableHandle aria-label="Resize file tree"/>
        <ResizablePanel initialSize={panelSizes()[1] ?? 0.7} class="flex-1 min-h-0 overflow-auto bg-zinc-950/30">
          <SelectedFilePanel isFileSelected={() => state.lastKnownFileNode?.kind === "file"} currentPath={state.lastKnownFilePath}/>
        </ResizablePanel>
      </Resizable>
    </div>);
};
