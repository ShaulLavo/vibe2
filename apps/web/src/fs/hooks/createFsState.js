...
export const createFsState = () => {
    ...
    const { selectedPath, setSelectedPath, activeSource, setActiveSource } = createSelectionState();
    const { selectedFileSize, setSelectedFileSize, selectedFilePreviewBytes, setSelectedFilePreviewBytes, selectedFileContent, setSelectedFileContent, selectedFileLoading, setSelectedFileLoading, error, setError, loading, setLoading, } = createFileDisplayState();
    const { backgroundPrefetching, setBackgroundPrefetching, backgroundIndexedFileCount, setBackgroundIndexedFileCount, lastPrefetchedPath, setLastPrefetchedPath, prefetchError, setPrefetchError, prefetchProcessedCount, setPrefetchProcessedCount, prefetchLastDurationMs, setPrefetchLastDurationMs, prefetchAverageDurationMs, setPrefetchAverageDurationMs, deferredMetadata, registerDeferredMetadata, clearDeferredMetadata, } = createPrefetchState();
    const { fileStats, setFileStats, clearParseResults } = createFileStatsState();
    const { pieceTables, setPieceTable, clearPieceTables } = createPieceTableState();
    const { fileHighlights, setHighlights, clearHighlights } = createHighlightState();
    ...
    const selectedNode = createMemo(() => tree ? findNode(tree, selectedPath()) : undefined);
    const lastKnownFileNode = createMemo((prev) => {
